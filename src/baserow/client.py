...
class BaseClient:
    ...
    def __init__(
        self, url: str, token: t.Optional[str] = None, jwt: t.Optional[str] = None
    ) -> None:
        if token and jwt:
    ...
    def _request(self, method: str, path: str, **kwargs) -> requests.Response:
        response = self._session.request(
            method, self._url + "/" + path.lstrip("/"), **kwargs
        )
        if (response.status_code // 100) in (4, 5) and response.headers.get(
            "Content-Type"
        ) == "application/json":
            data = response.json()
...
class BaserowClient(BaseClient):
    ...
    def token_refresh(self, token: str) -> t.Tuple[User, str]:
        ...
        response = self._request(
            "POST", "/api/user/token-refresh/", json=payload
        ).json()
        return databind.json.load(response["user"], User), response["token"]
    ...
    def update_database_table(self, table_id: int, name: str) -> Table:
        response = self._request(
            "PATCH", f"/api/database/tables/{table_id}/", json={"name": name}
        ).json()
        return databind.json.load(response, Table)

    def list_database_tables(self, database_id: int) -> t.List[Table]:
        response = self._request(
            "GET", f"/api/database/tables/database/{database_id}/"
        ).json()
    ...
    def create_database_table_row(
        self, table_id: int, record: t.Dict[str, t.Any]
    ) -> t.Dict[str, t.Any]:
        return self._request(
            "POST", f"/api/database/rows/table/{table_id}/", json=record
        ).json()

    def update_database_table_row(
        self,
        table_id: int,
        row_id: int,
    ...
    def get_database_table_row(
        self, table_id: int, row_id: int, user_field_names: bool = False
    ) -> t.Dict[str, t.Any]:
        params = {"user_field_names": "true"} if user_field_names else {}
        return self._request(
            "GET", f"/api/database/rows/table/{table_id}/{row_id}/", params=params
        ).json()

    # Extra

    def login(
        self, username: str, password: str, cache: t.Union[bool, str] = False
    ) -> User:
        """
